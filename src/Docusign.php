...
class Docusign
{
    ...
    public function getDocuments($envelopeId)
    {
        $request = $this->client->get('envelopes/' . $envelopeId . '/documents');
        return $envelope = $this->rawJson($request);
    }

    public function downloadDocuments($envelopeId, $download=true)
    {
        $docUri = "envelopes/$envelopeId/documents/combined";
        return ($download) ? $this->client->get($this->baseUrl . $docUri) : $this->baseUrl . $docUri;
    }
